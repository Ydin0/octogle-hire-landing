// Full country dial-code list for the phone prefix dropdown in the trial funnel.
// Each entry: iso2 (unique select value), country name, dial (E.164 prefix,
// what gets prepended to the number) and a flag emoji for the option label.
//
// `iso2` is the select value because several countries share one dial code
// (e.g. +1 across the US, Canada and the Caribbean); keying on iso2 keeps the
// option the user picked distinct while `dial` is what we actually send.
...
// A few high-traffic markets pinned to the top of the list. GB first: this is a
// UK product and ~all paid traffic is UK.
export const PRIORITY_ISO2 = ["GB", "IE", "US", "CA", "AU", "NZ"];
...
export const DIAL_CODES: DialCode[] = [
  ...
  { iso2: "AG", name: "Antigua and Barbuda", dial: "+1", flag: "🇦🇬" },
  ...
  { iso2: "BM", name: "Bermuda", dial: "+1", flag: "🇧🇲" },
  ...
  { iso2: "KY", name: "Cayman Islands", dial: "+1", flag: "🇰🇾" },
  ...
  { iso2: "DM", name: "Dominica", dial: "+1", flag: "🇩🇲" },
  ...
  { iso2: "GD", name: "Grenada", dial: "+1", flag: "🇬🇩" },
  ...
  { iso2: "JM", name: "Jamaica", dial: "+1", flag: "🇯🇲" },
  ...
  { iso2: "PR", name: "Puerto Rico", dial: "+1", flag: "🇵🇷" },
  ...
  { iso2: "KN", name: "Saint Kitts and Nevis", dial: "+1", flag: "🇰🇳" },
  { iso2: "LC", name: "Saint Lucia", dial: "+1", flag: "🇱🇨" },
  { iso2: "VC", name: "Saint Vincent and the Grenadines", dial: "+1", flag: "🇻🇨" },
  ...
  { iso2: "TT", name: "Trinidad and Tobago", dial: "+1", flag: "🇹🇹" },
  ...
  { iso2: "VA", name: "Vatican City", dial: "+379", flag: "🇻🇦" },
...
const BY_ISO2: Record<string, DialCode> = Object.fromEntries(
  DIAL_CODES.map((c) => [c.iso2, c]),
);

/** Dial prefix for an iso2 code, defaulting to +44 (GB). */
export function dialFor(iso2: string): string {
  return BY_ISO2[iso2]?.dial ?? "+44";
}

/**
 * Join a selected country prefix with a locally-typed number into one E.164-ish
 * string. If the user already typed a "+..." international number we trust it as
 * typed; otherwise we prepend the prefix and drop a single national trunk 0
 * (so UK "07737…" becomes "+44 7737…").
 */
export function composePhone(iso2: string, raw: string): string {
  const t = (raw || "").trim();
  if (!t) return "";
  if (t.startsWith("+")) return t.replace(/\s+/g, " ");
  const national = t.replace(/[^\d]/g, "").replace(/^0+/, "");
  if (!national) return "";
  return `${dialFor(iso2)} ${national}`;
}
