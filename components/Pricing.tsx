...
//
// Every price below is a placeholder until confirmed. `local` is the fully
// loaded UK cost for the same level; cite the source in `localNote`.
type Tier = {
  level: string;
  blurb: string;
  octogle: string;
        ...
        {TIERS.map((t) => (
          ...
          >
            ...
            <p className="mt-3 text-[14px] text-[var(--text-faint)]">
              UK hire, fully loaded: <Ph>{t.local}/mo</Ph>
            </p>
          </div>
        ))}
      </div>
      <p className="mt-5 text-[14px] text-[var(--text-faint)]">
        Full time, one client at a time. Monthly, no placement fee. UK figure
        includes salary, employer NI and pension:{" "}
        <Ph>source for UK figures</Ph>
      </p>
      <div className="mt-8">
